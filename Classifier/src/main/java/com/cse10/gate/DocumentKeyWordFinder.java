...
public class DocumentKeyWordFinder {
    ...
    private List annotationsTypesRequired;
    ...
    private void configureCorpusPipeLine() {
        if (!isCorpusPipeLineConfigured) {
            ...
            annotationsTypesRequired = new ArrayList();
            ArrayList<String> annotationType = new ArrayList();
            annotationType.add("Lookup");

            ListIterator annotationTypeIterator = annotationType.listIterator();
            while (annotationTypeIterator.hasNext()) {
                String annotation = (String) annotationTypeIterator.next();
                annotationsTypesRequired.add(annotation);
            }
            try {
    ...
    public boolean isKeyWordExist(String content) {
        ...
        if (content.length() > 0) {
            try {
                 ...
                 */
                if (this.annotationsTypesRequired != null) {

                    AnnotationSet defaultAnnotations = doc.getAnnotations();
                    Iterator annotationsRequiredIterator = this.annotationsTypesRequired.iterator();
                    while (annotationsRequiredIterator.hasNext()) {
                        ...
                        AnnotationSet annotationsOfThisType =
                                defaultAnnotations.get((String) annotationsRequiredIterator.next());
                ...
                Iterator annotationRequiredIterator = annotationsRequired.iterator();
                while (annotationRequiredIterator.hasNext()) {
                    /**
                     * extract all the annotations of each requested type and add
                     * them to the temporary set *
                     */
                    AnnotationImpl CurrentAnnot = (AnnotationImpl) annotationRequiredIterator.next();
